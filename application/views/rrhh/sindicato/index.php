...
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <div class="header-title">
      ...
      <small><?php echo details_module('Sindicato') ?></small>
      ...
      <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title"><span class="titulo_form_registro_mensual"></span> Sindicato
...
<script>
  ...
  function cargar_datos_mensuales(mes) {
    ...
    $.ajax({
      type: "post",
      url: "<?php echo base_url('rrhh/sindicato/get_data'); ?>",
      data: { mes },
      dataType: "json",
  ...
  $('#form_registrar_actualizar_registro_mensual').submit(function(e) {
    ...
    if (option == 'Registrar') { // Registrar
      msj_option = 'Se registrará un nuevo sindicato.';
      url_option = '<?php echo base_url('rrhh/sindicato/registrar'); ?>';
      ...
      if (verificar) {
        time_alert('error', 'Error!', 'Ya existe un sindicato registrado para el empleado seleccionado.', 2000);
        return;
      }
    } else { // Actualizar
      msj_option = 'Se actualizará los datos del sindicato.';
      url_option = '<?php echo base_url('rrhh/sindicato/actualizar'); ?>';
      ...
      .then((response) => {
        if (response) {
          ...
          $.ajax({
            ...
            success: function(response) {
              swloading.stop();
              time_alert('success', `${msj_alerta}!`, `Sindicato ${msj_alerta.toLowerCase()} correctamente.`, 2000)
                .then(() => {
    ...
    msg_confirmation('warning', '¿Está seguro?', 'No podrá revertir los cambios.')
      .then((response) => {
        if (response) {
          ...
          $.ajax({
            type: "post",
            url: "<?php echo base_url('rrhh/sindicato/eliminar'); ?>",
            data: { empleado, mes },
            dataType: "text",
            success: function(response) {
              swloading.stop();
  ...
  $('#btn_imprimir_registro_mensual').on('click', function() {
    ...
    const url_rm = "<?php echo base_url('rrhh/sindicato/reporte'); ?>?data=" + btoa(data);
    window.open(url_rm);
  });
