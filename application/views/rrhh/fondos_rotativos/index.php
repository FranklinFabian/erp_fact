...
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <div class="header-title">
      ...
      <small><?php echo details_module('Fondo Rotativo') ?></small>
        ...
        <div class="panel panel-bd lobidrag">
          ...
          <div class="panel-heading">
            <div class="panel-title">
              <h4><strong>Fondo Rotativo</strong></h4>
            </div>
          </div>
          <!-- Body -->
      ...
      <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title"><span class="titulo_form_registro_mensual"></span> Fondo Rotativo
...
<script>
  ...
  function cargar_datos_mensuales(mes) {
    ...
    $.ajax({
      type: "post",
      url: "<?php echo base_url('rrhh/fondoRotativo/get_data'); ?>",
      data: { mes },
      dataType: "json",
  ...
  $('#form_registrar_actualizar_registro_mensual').submit(function(e) {
    ...
    if (option == 'Registrar') { // Registrar
      msj_option = 'Se registrará un nuevo fondo rotativo.';
      url_option = '<?php echo base_url('rrhh/fondoRotativo/registrar'); ?>';
      ...
      if (verificar) {
        time_alert('error', 'Error!', 'Ya existe un fondo rotativo registrado para el empleado seleccionado.', 2000);
        return;
      }
    } else { // Actualizar
      msj_option = 'Se actualizará los datos del fondo rotativo.';
      url_option = '<?php echo base_url('rrhh/fondoRotativo/actualizar'); ?>';
      ...
      .then((response) => {
        if (response) {
          ...
          $.ajax({
            ...
            success: function(response) {
              swloading.stop();
              time_alert('success', `${msj_alerta}!`, `Fondo Rotativo ${msj_alerta.toLowerCase()} correctamente.`, 2000)
                .then(() => {
    ...
    msg_confirmation('warning', '¿Está seguro?', 'No podrá revertir los cambios.')
      .then((response) => {
        if (response) {
          ...
          $.ajax({
            type: "post",
            url: "<?php echo base_url('rrhh/fondoRotativo/eliminar'); ?>",
            data: { empleado, mes },
            dataType: "text",
            success: function(response) {
              swloading.stop();
  ...
  $('#btn_imprimir_registro_mensual').on('click', function() {
    ...
    const url_rm = "<?php echo base_url('rrhh/fondoRotativo/reporte'); ?>?data=" + btoa(data);
    window.open(url_rm);
  });
