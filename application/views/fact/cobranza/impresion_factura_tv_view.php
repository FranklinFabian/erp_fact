...
<body onload="imprimir()">
        ...
        <tr>
            ...
            <td width=35% class="" style="text-align:center">
                ...
                <table width=100% border=0 cellspacing=0 cellpadding=2>
                    ...
                    <tr>
                        <td style="width: 50%; text-align:left"><strong>CÓD. AUTORIZACIÓN:</strong></td> <td style="width: 50%; text-align:left"><?php echo $autorizacion['autorizacion'];//substr($factura['cuf'],0,17).'<br>'.substr($factura['cuf'],17,17).'<br>'.substr($factura['cuf'],34,17).'<br>'.substr($factura['cuf'],51).'<br>';?></td>
                    </tr>
                </table>
            </td>
        </tr>
    ...
    <table width=100% border=0 cellspacing=0 cellpadding=2>
        ...
        <tr>
            <td width=70%> <strong>FECHA: </strong> <?php echo (substr($factura['emision'], 0,10)).' Hr.:'.substr($factura['emision'], 11,8); ?></td>
    ...
    <table width=100% border=1 cellspacing=0 cellpadding=0>
        ...
        $msjMostrarDetalle.= '<tr><td>1</td><td>1</td><td align=left>UNIDAD (SERVICIOS)</td><td>CONSUMO TV CABLE</td><td align=right>'.number_format($consumo_periodo,2,',','.').'</td><td align=right>'.$lectura['devolucion'].'</td><td align=right>'.number_format($consumo_periodo-$lectura['imp_poten']-$lectura['imp_penal'] ,2,',','.').'</td>';
        ...
        $msjMostrarDetalle.= '
            ...
            <tr><td colspan=4 rowspan=9><strong>SON:</strong> '.$this->numeroaletras->convertir(($total_cobrar - $lectura['devolucion']),).' '.($centavos==0?'00':round($centavos,2)).'/100 Bolivianos</td><td colspan=2 align=right>SUB TOTAL Bs</td><td align=right>'.number_format((float)($total_cobrar), 2, ',', '.').'</td></tr>
            <tr><td colspan=2 align=right>(-) DESCUENTO Bs</td><td align=right>'.number_format((float)($lectura['devolucion']), 2, ',', '.').'</td></tr>
            <tr><td colspan=2 align=right>TOTAL Bs</td><td align=right><strong>'.number_format((float)($total_cobrar), 2, ',', '.').'</strong></td></tr>
            <tr><td colspan=2 align=right>MONTO GIFT CARD Bs</td><td align=right>0,00</td></tr>
        ...
        <tr>
            ...
            <td width=25% align="right">
                <?php
                    ...
                    $params['data'] = $this->config->item('nit').'|'.$factura['factura'].'|'.$autorizacion['autorizacion'].'|'.($factura['emision']).'|'.$total_cobrar.'|'.$total_iva.'|'.$factura['codcontrol'].'|'.$factura['nit'].'|0|0|'.$total_sin_iva.'|0';
                    $params['level'] = 'H';
                    $params['size'] = 4;
                    $params['savename'] = 'public/qr/'.$factura['idfactura'].'.png';
                    $this->ciqrcode->generate($params);
                ?>
                <img width="80" src="<?php echo base_url()?>public/qr/<?php echo $factura['idfactura']?>.png" alt="qr">
            </td>
        </tr>
    </table>
