  ...
  <ul class="dropdown-menu" role="menu">';
   ...
   $button .='</ul></div>';
    ...
    public function customer_list_pdf() {
        ...
        public function customer_transection_summary_ledger() {
        $result = array();
       ...
       public function customer_ledger_summary($customer_id,$start,$end) {
        $result = array();
        $this->db->select_sum('amount', 'total_credit');
        $this->db->from('customer_ledger');
        $this->db->where(array('d_c' => 'c','customer_id' => $customer_id, 'date >=' => $start, 'date <=' => $end));
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $result[] = $query->result_array();
        }
        $this->db->select_sum('amount', 'total_debit');
        $this->db->from('customer_ledger');
        $this->db->where(array('d_c' => 'd','customer_id' => $customer_id, 'date >=' => $start, 'date <=' => $end));
    ...
    public function customer_list_ledger() {
       ...
       public function customerledger_searchdata($customer_id, $start, $end) {
        $this->db->select('*');
        $this->db->from('customer_ledger');
        $this->db->where(array('customer_id' => $customer_id, 'date >=' => $start, 'date <=' => $end));
        $this->db->order_by('date', 'desc');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
...
//Retrieve customer All data 
 ...
 $query = $this->db->select('*')->from('customer_information')
   ...
   public function customer_csv_file() {
    ...
    public function advance_details($transaction_id){
