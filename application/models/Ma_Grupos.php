...
class Ma_Grupos extends CI_Model {
...
//    =========== its for unit check and insert start ===============
    public function insert($data) {
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->db->insert('almacen_grupo', $data);
            return TRUE;
        }
    }
...
//    =========== its for unit list show start ===============
    public function list() {
        $this->db->select('g.*, a.nombre almacen');
        $this->db->from('almacen_grupo g');
...
//    =========== its for unit update start  ===============
    public function update($data, $id) {
        $this->db->where('id', $id);
        $this->db->update('almacen_grupo', $data);
        return TRUE;
    }
