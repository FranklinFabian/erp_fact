...
class Mga_Certificados extends CI_Model {
    ...
    public function getList($postData=null){
         ...
         foreach($records as $record ){
          ...
          $base_url = base_url();
             ...
             $button .=' <a href="'.$base_url.'Cga_certificado/update_form/'.$record->id.'" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="top" title="Ingresar"><i class="fa fa-eye" aria-hidden="true"></i></a>';

             $button .=' <a href="'.$base_url.'Cga_certificado/print/'.$record->id.'" class="btn btn-success btn-xs" data-toggle="tooltip" data-placement="right" title="Imprimir" target="_blank" ><i class="ti-printer" aria-hidden="true"></i></a>';
    ...
    //Reporte pdf ficha
    public function vista_cliente($id) {
        $this->db->select('gc.*,
                DATE_FORMAT(gc.fecha_registro, "%d-%m-%Y") as fecha_registro_formato,
                DATE_FORMAT(gc.fecha_nacimiento, "%d-%m-%Y") as fecha_nacimiento_formato,
                DATE_FORMAT(gc.fecha_socio, "%d-%m-%Y") as fecha_socio_formato,
    ...
    public function reporte_cliente($id) {
        $this->db->select('gc.*,
                DATE_FORMAT(gc.fecha_registro, "%d-%m-%Y") as fecha_registro_formato,
                DATE_FORMAT(gc.fecha_nacimiento, "%d-%m-%Y") as fecha_nacimiento_formato,
                DATE_FORMAT(gc.fecha_socio, "%d-%m-%Y") as fecha_socio_formato,
                ...
                gec.nombre as estado_civil,
                gd.nombre as departamento,
                gs.costo as monto_certificado,
                gcert.codigo as codigo_certificado');
        $this->db->from('ga_cliente gc');
        $this->db->join('ga_ocupacion go','go.id = gc.id_ocupacion','left');
        $this->db->join('ga_nivel_educacion gne','gne.id = gc.id_nivel_educacion','left');
        $this->db->join('ga_profesion gp','gp.id = gc.id_profesion','left');
        ...
        $this->db->join('ga_certificado gcert','gcert.id_suscripcion = gs.id','left');
        $this->db->where('gcert.id', $id);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
    ...
    public function reporte_certificado($id) {
        ...
        $this->db->join('ga_cliente gcl','gcl.id = gs.id_cliente','left');
        $this->db->where('gcl.id', $id);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
