...
class Medicion extends CI_Model {
    ...
    public function getList($postData=null){
        ...
        if($searchValue != ''){
            $searchQuery = " (
            am.id like '%".$searchValue."%'
            or am.nombre like'%".$searchValue."%'
            ) ";
        }

        ## Total number of records without filtering
        $this->db->select('count(*) as allcount');
        $this->db->from('adq_medicion am');
        if($searchValue != '')
        ...
        foreach($records as $record ){
            ...
            if($this->permission1->method('administrar_adquisiciones_medicion','delete')->access()) {
