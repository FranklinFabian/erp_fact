...
class Zona extends CI_Model {
    ...
    public function getList($postData=null){
        ...
        if($searchValue != ''){
            $searchQuery = " (
            az.id like '%".$searchValue."%'
            or az.nombre like'%".$searchValue."%'
        ...
        $this->db->from('adq_zona az');
        $this->db->join('adq_localidad al','al.id = az.id_localidad','left');
        if($searchValue != '')
        ...
        foreach($records as $record ){
            ...
            if($this->permission1->method('administrar_adquisicion_zona','delete')->access()) {
