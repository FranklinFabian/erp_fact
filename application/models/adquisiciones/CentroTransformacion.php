...
class CentroTransformacion extends CI_Model {
    ...
    public function getList($postData=null){
        ...
        $this->db->from('adq_centro_transformacion act');
        if($searchValue != '')
        ...
        foreach($records as $record ){
            ...
            if($this->permission1->method('administrar_adquisiciones_centrotransformacion','delete')->access()) {
    ...
    public function retrieve_datamodal($id) {
        $this->db->select('*');
        $this->db->from('adq_centro_transformacion');
        $this->db->where('id', $id);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
