...
class Consumidor extends CI_Model {
    ...
    public function getList($postData=null){
        ...
        if($searchValue != ''){
            $searchQuery = " (
            ac.id like '%".$searchValue."%'
            or ac.nombre like'%".$searchValue."%'
            ) ";
        }

        ## Total number of records without filtering
        $this->db->select('count(*) as allcount');
        $this->db->from('adq_consumidor ac');
        if($searchValue != '')
        ...
        foreach($records as $record ){
            ...
            if($this->permission1->method('administrar_adquisiciones_consumidor','delete')->access()) {
