...
class Lsupplier {
    ...
    public function supplier_ledger_info($supplier_id) {
        ...
        $CI->load->model('Web_settings');
        $receiptdata      = $CI->Suppliers->advance_details($receiptid);
        $supplier_details = $CI->Suppliers->supplier_personal_data($receiptdata->supplier_id);
        $currency_details = $CI->Web_settings->retrieve_setting_editdata();
        ...
        $chapterList = $CI->parser->parse('supplier/supplier_advance_receipt', $data, true);
        return $chapterList;
    }
