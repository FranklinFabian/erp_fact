...
class Cma_pedidotecnico extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('Ma_Pedidostecnicos');
    ...
    public function index() {
        ...
        $CI->load->library('lma_pedidotecnico');
        $content = $CI->lma_pedidotecnico->add_form();
    ...
    public function insert() {
        ...
        $correlativo = $this->db->get_where("almacen_correlativo",array('nombre' => 'pedido_tecnico'))->row_array();
        ...
        $this->db->update('almacen_correlativo', $correlativo, 'nombre = "pedido_tecnico"');


        if (isset($_POST['add'])) {
            $this->session->set_userdata(array('message' => 'Insertado exitosamente.'));
            redirect(base_url('Cma_pedidotecnico/update_form/' . $id_cabecera ));
            exit;
        } else {
            redirect(base_url('Cma_pedidotecnico/administrar'));
            exit;
        }
    }
    ...
    public function update_form($id) {
        ...
        $CI->auth->check_admin_auth();
        $CI->load->library('lma_pedidotecnico');
        $content = $CI->lma_pedidotecnico->edit_data($id);
        $this->template->full_admin_html_view($content);
    }
    ...
    public function update() {
        ...
        $this->db->update('cabecera_pedido', $cabecera, "id =". $id);

        $this->session->set_userdata(array('message' => display('successfully_updated')));
        redirect(base_url('Cma_pedidotecnico/update_form/' . $id));
    ...
    public function administrar()
    {
        ...
        $CI->load->library('lma_pedidotecnico');
        $CI->load->model('Ma_Pedidostecnicos');
        $content =$this->lma_pedidotecnico->list();
        $this->template->full_admin_html_view($content);
    }
    ...
    public function dataList(){
        ...
        $data = $this->Ma_Pedidostecnicos->getList($postData);
    ...
    public function print($id){
        ...
        $cabecera = $CI->Ma_Pedidostecnicos->impresion_cabecera($id);
        $detalle = $CI->Ma_Pedidostecnicos->impresion_detalle($id);
        ...
        $content = $this->parser->parse('almacen/pedidotecnico/formulario_pdf', $data, true);
        $pdf = new DOMPDF();
        $pdf->set_option('enable_php', TRUE);
        $pdf->set_paper('letter', 'portrait');
        $pdf->load_html($content);
        $pdf->render();
        $pdf->stream('almacen_pt_'.$id, array("Attachment"=>1));
    }
    ...
    public function dataTable($postData=null){
        ...
        foreach($records as $record ){
            ...
            if($this->permission1->method('administrar_almacen_pedido_tecnico','delete')->access()) {
    ...
    public function modal_edit(){
        ...
        if($type == 'update'){
            $data['item'] = $this->Ma_Pedidostecnicos->retrieve_datamodal($id)[0];
        ...
        $this->load->view('almacen/pedidotecnico/modal', $data);
    }
