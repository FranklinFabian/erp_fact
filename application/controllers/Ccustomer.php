    ...
    $customer_headcode = $coainfo->HeadCode;
      ...
      'IsAppove'       =>  1
                   ...
                   $this->db->insert('acc_transaction',$cc);
                    redirect(base_url('Ccustomer/customer_advancercpt/'.$transaction_id));

  }


   public function customer_advancercpt($receiptid) {
        $CI = & get_instance();
        $CI->auth->check_admin_auth();
        $CI->load->library('lcustomer');
        $content = $CI->lcustomer->advance_details_data($receiptid);
        $this->template->full_admin_html_view($content);
    }
}
