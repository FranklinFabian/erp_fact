...
class SisTipoMedicion extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $crud->set_table('sis_tipos_mediciones');
        $crud->set_subject('Tipo de Medición', 'Tipo de Mediciones');
        $crud->columns('Tipo_Medicion', '_Creado_Por');
        // editando el registro
        $state = $crud->getState();
        if ($state == 'edit' || $state == 'update' || $state == 'update_validation') {
            $crud->fields('Tipo_Medicion', '_Actualizado_Por');
        }
        if ($state == 'ajax_list' || $state == 'list' || $state == 'success') {
            ...
            $crud->fields('Tipo_Medicion', '_Creado_Por');
            $crud->set_rules('Tipo_Medicion', 'Tipo_Medicion', 'required');
        }
