...
class Envio_facturas_13 extends CI_Controller{

  // public function index(){
  //   if(isAdmin()){
      
  //     $periodo_act = $this->periodos_model->get_activo();
  //     $facturas_array = $this->generar_facturas_13_model->get_lecturas_periodo($periodo_act['idperiodo'], 1);
  //     $cantidad_restante = count($facturas_array);
  ...
  public function generar_pdf_enviar(){
    if(isAdmin()){
      ...
      if(is_null($periodo_act))
        ...
        $facturas_array = $this->generar_facturas_13_model->get_facturas_enviadas($periodo_act['idperiodo'], 1);
