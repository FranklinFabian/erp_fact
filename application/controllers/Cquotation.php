...
//    ========== its for  insert_quotation =============
    public function insert_quotation() {
        ...
        $num_column = count($tablecolumn)-4;
            ...
            'item_total_amount'   => $this->input->post('grand_total_price'),
            'item_total_dicount'  => $this->input->post('total_discount'),
            'item_total_tax'      => $this->input->post('total_tax'),
            'service_total_amount'=> $this->input->post('grand_total_service_amount'),
            'service_total_discount'=> $this->input->post('totalServiceDicount'),
            'service_total_tax'   => $this->input->post('total_service_tax'),
            'quot_dis_item'       =>$this->input->post('invoice_discount'),
            'quot_dis_service'    =>$this->input->post('service_discount'),
            ...
            if ($result == TRUE) {
                ...
                $descrp       = $this->input->post('desc');
                $item_rate    = $this->input->post('product_rate');
                $item_supp_rate= $this->input->post('supplier_price');
                $item_qty     = $this->input->post('product_quantity');
                $item_dis_per = $this->input->post('discount');
                $item_total_discount = $this->input->post('discount_amount');
            ...
            $taxdata['customer_id'] = $this->input->post('customer_id');
            $taxdata['date']        = (!empty($this->input->post('qdate'))?$this->input->post('qdate'):date('Y-m-d'));
            $taxdata['relation_id'] = 'item'.$this->input->post('quotation_no');
            $this->db->insert('quotation_taxinfo',$taxdata);
                ...
                $service_qty  = $this->input->post('service_quantity');
                $service_dis_per = $this->input->post('sdiscount');
                $service_total_discount = $this->input->post('sdiscount_amount');
                $totalservicep = $this->input->post('total_serviceprice');
                $service_tax     = $this->input->post('stax');
            ...
            $servicetaxinfo['customer_id'] =$this->input->post('customer_id');
            $servicetaxinfo['date']        = (!empty($this->input->post('qdate'))?$this->input->post('qdate'):date('Y-m-d'));
            $servicetaxinfo['relation_id'] = 'serv'.$this->input->post('quotation_no');
...
//    ============= its for  manage quotation ============
    ...
    public function add_quotation_to_invoice()
    {
        ...
        $squantity    = $this->input->post('service_quantity');
        $tablecolumn  = $this->db->list_fields('tax_collection');
        $num_column   = count($tablecolumn)-4;
       ...
       $transection_id = $this->auth->generator(15);
        ...
        $datainvmain = array(
            ...
            'date'            => (!empty($this->input->post('qdate'))?$this->input->post('qdate'):date('Y-m-d')),
            'total_amount'    => $this->input->post('grand_total_price'),
            'total_tax'       => $this->input->post('total_tax'),
            'invoice'         => $this->number_generator(),
            'invoice_details' => (!empty($this->input->post('details'))?$this->input->post('details'):'From Quotation'),
 ...
 $banksummary = array(
            ...
            'dr'            =>  $this->input->post('grand_total_price'),
            'cr'            =>  null,
    ...
    $cosdr = array(
      ...
      'IsAppove'       => 1
        ...
        $this->db->insert('customer_ledger', $customerledgerdebit);
        $this->db->insert('acc_transaction',$coscr);
        $this->db->insert('acc_transaction',$cosdr);  
        $this->db->insert('acc_transaction',$pro_sale_income);
        $this->db->insert('acc_transaction',$cuscredit);
        $this->db->insert('customer_ledger', $customercreditledger);
        ...
        for ($i = 0, $n = count($p_id); $i < $n; $i++) {
            ...
            $invoiceDetails = array(
                ...
                'discount_per'       => $disper,
                'tax'                => $tax,
                'paid_amount'        => $this->input->post('grand_total_price'),
                'due_amount'         => '',
                'supplier_rate'      => $supplier_rate,
                'total_price'        => $total_price,
                'status'             => 1
        ...
        ##==== SERVICE PART START ====###
         $customerledger_dbt = array(
            'transaction_id'  => $transection_id,
            'customer_id'     => $customer_id,
            'invoice_no'      => $invoice_id,
            'date'            => (!empty($this->input->post('qdate'))?$this->input->post('qdate'):date('Y-m-d')),
            'amount'          => $this->input->post('grand_total_service_amount'),
            'description'     => 'Service from Quotation',
            'status'          => 1,
            'd_c'             => 'd'
            );
            ...
            'customer_id'     => $customer_id,
            'date'            => (!empty($this->input->post('qdate'))?$this->input->post('qdate'):date('Y-m-d')),
            'total_amount'    => $this->input->post('grand_total_service_amount'),
            'total_tax'       => $this->input->post('total_service_tax'),
            'voucher_no'      => $invoice_id,
            'details'         => (!empty($this->input->post('details'))?$this->input->post('details'):'Service From Quotation'),
            'invoice_discount'=> $this->input->post('service_discount'),
            'total_discount'  => $this->input->post('totalServiceDicount'),
            'shipping_cost'   => '',
            'paid_amount'     => $this->input->post('grand_total_service_amount'),
            'due_amount'      => 0,
            'previous'        => '',
      ...
      'Debit'          =>  $this->input->post('grand_total_service_amount'),
      'Credit'         =>  0,
      'IsPosted'       =>  1,
      'CreateBy'       =>  $createby,
      'CreateDate'     =>  $createdate,
...
$service_income = array(
      ...
      'Debit'          =>  0,
      'Credit'         =>  $this->input->post('grand_total_service_amount'),
      'IsPosted'       =>  1,
      'CreateBy'       =>  $createby,
      'CreateDate'     =>  $createdate,
      'IsAppove'       =>  1
...
$cosdr_service = array(
 ...
 $banksummaryservice = array(
            ...
            'description'   =>  'product sale',
            'deposite_id'   =>  $invoice_id,
            'dr'            =>  $this->input->post('grand_total_service_amount'),
            'cr'            =>  null,
            'ammount'       =>  $this->input->post('grand_total_service_amount'),
            'status'        =>  1
        
        );

 if (!empty($squantity)) {
    $this->db->insert('customer_ledger', $customerledger_dbt);
    ...
    $this->db->insert('customer_ledger', $customerledger_credt);
...
if($this->input->post('paytype') == 2){
    ...
    $this->db->insert('bank_summary', $banksummaryservice);
        ...
        $qty                 = $this->input->post('service_quantity');
        $srate               = $this->input->post('service_rate');
        $serv_id             = $this->input->post('service_id');
        $total_serviceamount = $this->input->post('total_serviceprice');
        $sdiscount_rate      = $this->input->post('sdiscount_amount');
        $sdiscount_per       = $this->input->post('sdiscount');
        $tax_amount          = $this->input->post('stax');
        $invoice_description = $this->input->post('details');
