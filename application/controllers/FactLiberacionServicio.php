...
class FactLiberacionServicio extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $crud->set_table('fact_liberacion_servicios');
        $crud->set_subject('Liberación de Servicio', 'Liberación de Servicios');
        $crud->columns('Liberacion_Servicio', '_Creado_Por');
        // editando el registro
        $state = $crud->getState();
        if ($state == 'edit' || $state == 'update' || $state == 'update_validation') {
            $crud->fields('Liberacion_Servicio', '_Actualizado_Por');
        }
        if ($state == 'ajax_list' || $state == 'list' || $state == 'success') {
            ...
            $crud->fields('Liberacion_Servicio', '_Creado_Por');
            $crud->set_rules('Liberacion_Servicio', 'Liberacion_Servicio', 'required');
        }
