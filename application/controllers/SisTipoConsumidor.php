...
class SisTipoConsumidor extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $crud->set_table('sis_tipos_consumidores');
        $crud->set_subject('Tipo de Consumidor', 'Tipo de Consumidores');
        $crud->columns('Tipo_Consumidor', '_Creado_Por');
        // editando el registro
        $state = $crud->getState();
        if ($state == 'edit' || $state == 'update' || $state == 'update_validation') {
            $crud->fields('Tipo_Consumidor', '_Actualizado_Por');
        }
        if ($state == 'ajax_list' || $state == 'list' || $state == 'success') {
            ...
            $crud->fields('Tipo_Consumidor', '_Creado_Por');
            $crud->set_rules('Tipo_Consumidor', 'Tipo_Consumidor', 'required');
        }
